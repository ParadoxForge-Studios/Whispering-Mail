📮 Whispering Mail 
A cozy postal puzzle adventure — built in 30 days Whispering Mail is a short, atmospheric puzzle-adventure where you play as the mail carrier of a quiet little village. Every day you receive a stack of mysterious letters filled with cryptic hints, symbols, and half-revealed names. Your task is to discover who each letter truly belongs to through conversations, observation, and small puzzles.

🕒 This game was developed as a fully playable 30-day student project, with a strong focus on small scope, mood, and polished core mechanics. 
No combat. No stress. Just calm exploration, cozy mystery, and gentle storytelling.

🌿 Gameplay 🧩 Core Loop Pick up your mail at the post office Inspect the letters (names, symbols, cryptic hints). 
Explore the village and talk to residents Solve small puzzles to identify the recipient. 
Deliver the correct letter --> receive a short reaction or mini cutscene. 
When all letters are delivered --> the next day begins. 
The challenge gradually increases over 6–7 in-game days.

🎭 Characters The village features 8 unique residents, each with their own mini-puzzle: 
🧓 The forgetful grandpa by the pond 
👩‍🍳 The baker with a cryptic recipe 
🐱 The cat owner with a runaway cat 
🧑‍🏫 The strict schoolteacher full of symbols 
🛠️ The carpenter with a sorting puzzle 
🪴 The plant lover who speaks in leaf shapes 
🐕 The dog trainer who leads you by sound 
👑 The mayor behind guarded doors

Each successful delivery comes with a small, personal payoff.

🏡 The World A compact, hand-crafted village with around 10 buildings:
- Post office (starting point)
- Bakery
- School
- Park with pond
- Carpenter’s workshop
- Greenhouse
- Town hall
- A few simple houses Not every building has an interior — the focus is on atmosphere, not scale.

🎮 Mechanics 📨 
Letter Inspection Rotate & zoom Highlight symbols Discover small visual clues

🧠 Mini Puzzles 
Following animals 
Memory & sorting puzzles 
Simple crafting 
Sound-based clues 

Additional systems:
🗺️ Simple minimap 
🌤️ Day–night cycle 
💬 Text-based dialogue system

🔄 Progression 
Day 1: 3 letters 
Day 2: 4 letters 
Day 3–4: 4–5 letters 
Day 5–7: Up to 6 letters

Each day adds: One new puzzle A small narrative layer New dialogue Total playtime: ~15–20 minutes

⏱️ Scope (30-Day Student Project) Whispering Mail was fully designed and developed within 30 days as a solo/student project. The scope was carefully limited to ensure a complete, polished experience within that time frame.

✅ Included Small hand-crafted village 8 NPCs 5–6 mini-games Simple animations Text-based dialogue 1 music track + ambient sounds

❌ Not Included Combat Inventory systems Complex AI Voice acting Large open world

🛠️ Tech Stack Engine: 
Unity Language: C# 
Art Style: Low-poly / stylized 
Platform: Windows (Itch.io)

📦 Build
(To be added upon release)

📜 License
TBD
